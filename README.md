...
音色库试听和语音服务试音使用临时播放队列，不会覆盖朗读页已经完成的分轨与音频。切回朗读页时会恢复当前消息的完整播放队列。

音色库分为可折叠的“我的音色”和“插件内置音色”；克隆及手动添加的音色会集中显示在“我的音色”。自动分组池区分男童、女童、青年、成熟与老年声线，并在年龄和性别匹配后优先选择符合 `clear`、`warm`、`cold`、`calm`、`deep`、`bright`、`soft` 气质标签的音色。职业或身份不单独建立分组，而由分轨导演映射为稳定声线气质，避免分组无限膨胀。
...
- 「调整」面板会显示模型、实际 voice_id、情绪、语速、固定音高、导演置信度与强度、拟声标签、音频格式及实际送入 MiniMax 的文本；可手动修改本段音色、情绪和语速后强制重新生成。
